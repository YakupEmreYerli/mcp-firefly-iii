...
export const currencyOperations: Record<string, Operation> = {
  ...
  delete: defineOperation({ description: "Delete a currency.", access: "write", input: currencyCode, handler: async ({ code }, c) => { await c.del(`/currencies/${code}`); return { deleted: true, code }; } }),
...
export const exchangeRateOperations: Record<string, Operation> = {
  ...
  delete: defineOperation({ description: "Delete an exchange rate.", access: "write", input: id, handler: async ({ id }, c) => { await c.del(`/exchange-rates/${id}`); return { deleted: true, id }; } }),
};
...
export const attachmentOperations: Record<string, Operation> = {
  ...
  delete: defineOperation({ description: "Delete an attachment.", access: "write", input: id, handler: async ({ id }, c) => { await c.del(`/attachments/${id}`); return { deleted: true, id }; } }),
...
export const recurringOperations: Record<string, Operation> = {
  ...
  delete: defineOperation({ description: "Delete a recurring transaction.", access: "write", input: id, handler: async ({ id }, c) => { await c.del(`/recurrences/${id}`); return { deleted: true, id }; } }),
};
