...
export const tagOperations: Record<string, Operation> = {
  ...
  delete: defineOperation({ description: "Delete a tag.", access: "write", input: z.object({ id: entityId }).strict(), handler: async ({ id }, client) => { await client.del(`/tags/${id}`); return { deleted: true, id }; } }),
